...
const DrawBoard = () => {
  ...
  const boardLeft =
    (gameConfig.width - (3 * this.getPlatformSize() + 2 * this.spaceSize)) / 2
  const boardTop =
    (gameConfig.height - (3 * this.getPlatformSize() + 2 * this.spaceSize)) / 2
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    children[i].once('pointerdown', () =>
      this.drawSymbol(children[i].x, children[i].y, this.getPlatformSize())
    )
  }
}
